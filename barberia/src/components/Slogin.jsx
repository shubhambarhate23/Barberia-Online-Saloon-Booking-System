...
const Slogin = () => {
    ...
    useEffect(() => {
        fetch("http://localhost:8080/api/v1/allShop")
            .then(res => res.json())
   ...
   // console.log(pass.value);
    ...
    user.map(val => {
        if (val.email === id.value && val.password === pass.value) {
            ...
            fetch("http://localhost:8080/api/v1/shoplogin/" + obj.password)
                .then(res => res.json())
                .then((result) => {
                    setUserenter(result);
                });
            toast.success("Login Successfull");
            navigate('/ShopProfile/'+val.email);
        }
       // console.log(verify);
    }
    )
                ...
                <div className="row">
                    <div className="col-md-3 offset-md-10 mt-5">
                        <div className="card mt-5">
                            <div className="card-header text-center fs-3 text-success">Shop Login</div>
